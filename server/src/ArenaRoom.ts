...
export class ArenaRoom extends Room<ArenaState>{
 ...
 private attacks=new Map<string,number>(); private votes=new Map<string,number>(); private joinOrder=new Map<string,number>(); private seatCounter=0;
 ...
 onJoin(c:Client,o:{name?:string}){
  ...
  p.facing=p.team==="A"?1:-1;p.weapon="bow";p.maxHp=HP[p.weapon];p.hp=p.maxHp;
 ...
 onLeave(c:Client){this.inputs.delete(c.sessionId);this.attacks.delete(c.sessionId);this.joinOrder.delete(c.sessionId);const t=this.timers.get(c.sessionId);if(t)clearTimeout(t);this.timers.delete(c.sessionId);this.state.players.delete(c.sessionId);if(this.state.players.size<MAX&&!DEV)this.state.phase="waiting";}
 private tick(d:number){
  ...
  for(const [id,p] of this.state.players){
   ...
   if(dx)p.facing=dx>0?1:-1;p.x=clamp(p.x+dx/l*s,b.left,b.right);p.y=clamp(p.y+dy/l*s,b.top,b.bottom);
 ...
 private kill(a:PlayerState,t:PlayerState,tid:string){
  ...
  const timer=setTimeout(()=>{if(!this.state.players.has(tid))return;if(t.lives<=0)t.lives=1;t.maxHp=t.weakened?1:HP[t.weapon];t.hp=t.maxHp;const b=this.bounds();t.x=b.left+Math.random()*(b.right-b.left);t.y=b.top+Math.random()*(b.bottom-b.top);t.alive=true;this.timers.delete(tid);},delay);
 ...
 private finish(){if(this.state.phase==="finished")return;this.state.phase="finished";const players=[...this.state.players.entries()];const sorted=[...players].sort((a,b)=>b[1].kills-a[1].kills||(this.joinOrder.get(a[0])||0)-(this.joinOrder.get(b[0])||0));const teamA=sorted.filter(([,p])=>p.team==="A"),teamB=sorted.filter(([,p])=>p.team==="B");const bestA=teamA[0],bestB=teamB[0];let leaders=[bestA,bestB].filter(Boolean);const top2=sorted.slice(0,2);if(top2.length===2&&top2[0][1].team===top2[1][1].team){const other=top2[0][1].team==="A"?bestB:bestA;if(top2[1][1].kills-(other?.[1].kills??0)>=2)leaders=top2;}const ids=new Set(leaders.map(([id])=>id));this.broadcast("round-result",leaders.map(([,p])=>({name:p.name,team:p.team,kills:p.kills})));for(const [id] of players)if(!ids.has(id)){const c=this.clients.find(x=>x.sessionId===id);c?.leave(4000);}setTimeout(()=>{this.state.players.forEach((p,id)=>{if(!ids.has(id))return;p.kills=0;p.lives=2;p.weakened=false;p.maxHp=HP[p.weapon];p.hp=p.maxHp;p.alive=true;});this.resetRound();if(this.state.players.size===MAX||(DEV&&this.state.players.size>=1))this.state.phase="playing";},1500);}
}