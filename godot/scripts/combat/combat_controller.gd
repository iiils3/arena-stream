...
var hit_count:=0
var last_attacker:=-1
func equip(data:ArenaWeaponData)->void: weapon=data
func tick(delta:float)->void:
    if attack_cooldown>0.0: attack_cooldown=maxf(0.0,attack_cooldown-delta)
...
func try_attack()->bool:
    if state!=State.READY or attack_cooldown>0.0: return false
    state=State.ACTIVE
...
func apply_hit(attacker_id:int,attacker_team:int,weapon_data:ArenaWeaponData,knockback_amount:float)->bool:
    if state==State.DEFEATED or attacker_team==team: return false
